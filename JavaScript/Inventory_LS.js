...
for (let i in Current_Inventory){
    ...
    Inventory_Item_Cloned.Collection_Name = Current_Inventory[i].Collection_Name
...
const Inventory_Item_Info_Rarity = document.querySelector(".Inventory_Item_Info_Rarity");
const Inventory_Item_Info_Description = document.querySelector(".Inventory_Item_Info_Description")
const Inventory_Item_Info_Collection = document.querySelector(".Inventory_Item_Info_Collection") ;
const Inventory_Item_Info_Collection_Container = document.querySelector(".Inventory_Item_Info_Collection_Container")
const Inventory_Item_Buttons = document.querySelector(".Inventory_Item_Buttons")
...
Inventory_Skins.forEach(function (Item){
    Item.onmouseenter = function(){
        ...
        let newPosX = Item.offsetLeft + Item.offsetWidth + Inventory_Item_Info.offsetWidth/10;
        let newPosY = Item.offsetTop - Item.parentNode.scrollTop;
        //If box goes over the border
        if(newPosY + Inventory_Item_Info.offsetHeight - Item.offsetHeight > Item.parentNode.offsetHeight && newPosX + Inventory_Item_Info.offsetWidth > Item.parentNode.offsetWidth){
            Corner_Function(corner4)
            newPosY = Item.offsetTop - Item.parentNode.scrollTop - Inventory_Item_Info.offsetHeight + Item.offsetHeight
            newPosX = Item.offsetLeft - Inventory_Item_Info.offsetWidth - Inventory_Item_Info.offsetWidth/10
        }
        if (newPosX + Inventory_Item_Info.offsetWidth > Item.parentNode.offsetWidth && newPosY + Inventory_Item_Info.offsetHeight - Item.offsetHeight < Item.parentNode.offsetHeight){
            newPosX = Item.offsetLeft - Inventory_Item_Info.offsetWidth - Inventory_Item_Info.offsetWidth/10
        ...
        //Setting Styles
        document.querySelector(".Inventory_Item_Info_Name").textContent = Item.ItemName 
        Inventory_Item_Info_Description.textContent = Item.Description
        Inventory_Item_Info_Collection.textContent = "The " + Item.Collection_Name.replace("Case" , "") + " Collection";
        ...
        for(let i = 0 ; i < Case_Rarity_Skins_Array[3].length ; i++){
            Current_P += 1
            Inventory_Item_Info_Collection_Container.children[i].textContent = Object.values(Case_Rarity_Skins_Array[3])[i].name
           
            Inventory_Item_Info_Collection_Container.children[i].style.color = "rgb(61, 87, 195)"
        }
        for(let i = 0 ; i < Case_Rarity_Skins_Array[2].length ; i++){
            if (Current_P < 8){
            Inventory_Item_Info_Collection_Container.children[Current_P].textContent = Object.values(Case_Rarity_Skins_Array[2])[i].name
            Inventory_Item_Info_Collection_Container.children[Current_P].style.color = "rgb(107, 72, 195)"
            Current_P += 1
            }
        }
        for(let i = 0 ; i < Case_Rarity_Skins_Array[1].length ; i++){
            if (Current_P < 8){
            Inventory_Item_Info_Collection_Container.children[Current_P].textContent = Object.values(Case_Rarity_Skins_Array[1])[i].name
            Inventory_Item_Info_Collection_Container.children[Current_P].style.color = "rgb(166, 60, 174)"
            Current_P += 1
            }
        }
       ...
       if (Total_Collection_Skins-8 != 1){
        Inventory_Item_Info_Collection_Container.children[8].textContent = `and ${Total_Collection_Skins-8} more items...`
       }else{
        Inventory_Item_Info_Collection_Container.children[8].textContent = `and ${Total_Collection_Skins-8} more item...`
       }
        ...
        function setBackgroundRarityColor (Color){
            Inventory_Item_Info_Rarity.style.background =
             `linear-gradient(90deg,rgba(100, 100, 100,0.01) 0%, ${Color} 20%, ${Color} 80% , rgba(0, 0, 0,0.01) 100%)
    
             `;
        }
        if (Item.Rarity == Rarity_Array[0]) setBackgroundRarityColor(Rarity_Colors[0]);
        if (Item.Rarity == Rarity_Array[1]) setBackgroundRarityColor(Rarity_Colors[1]);
        if (Item.Rarity == Rarity_Array[2]) setBackgroundRarityColor(Rarity_Colors[2]);
        if (Item.Rarity == Rarity_Array[3] 
        ||
        Item.Rarity == Rarity_Array[4]) setBackgroundRarityColor(Rarity_Colors[3]);
        //Move the Info Box
    ...
    Item.onclick = function(e){
        ...
        var rect = Item.parentNode.parentNode.getBoundingClientRect();
        var x = e.clientX - rect.left;
        var y = e.clientY - rect.top - Inventory_Item_Buttons.offsetHeight/2;
        Inventory_Item_Info.style.display = "none"
